...
public abstract class AbstractRestHandler<T> implements Handler {
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (target.startsWith(statusPath)) {
            ...
            writeStatusJson(writer, request);
    ...
    void writeStatusJson(JsonWriter writer, HttpServletRequest request) {
        try (JsonWriter root = writer.startObject()) {
            try (JsonWriter statusArray = root.startArrayAttribute(statusJsonArrayName)) {
                for (Map.Entry<String, T> entry : itemMap.entrySet()) {
                    try (JsonWriter e = statusArray.startObject()) {
                        e.addAttribute("id", entry.getKey());
                        writeJsonItemValues(e, entry.getValue(), request);
                    }
                }
            }
        }
    ...
    abstract void writeJsonItemValues(JsonWriter jw, T item, HttpServletRequest request);
